...
const notInit = () => {
  throw new Error('Core Service is not initilized');
};

let checkLoggedIn: () => void = notInit;
let action: actionFunction = notInit;
let downloadBackup: () => Promise<void> = notInit;
let restoreBackup: () => Promise<void> = notInit;
...
export async function init() {
  if (window.useLocalCore) {
    ...
    const { retrocycle } = await import('./cycle');
